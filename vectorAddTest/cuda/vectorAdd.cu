...
main(void)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to launch vectorAddTest kernel (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
    ...
    printf("vectorAddTest cuda total time (ms): %f \n", (float)total_t / CLOCKS_PER_SEC * 1000);
    printf("vectorAddTest cuda execution time (ms): %f \n", (float)execution_t / CLOCKS_PER_SEC * 1000);
